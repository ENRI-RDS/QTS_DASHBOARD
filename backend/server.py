...
# in Master.csv hanno la pratica di autorizzazione gestita là, non da Sertori
# su QTS. ENRI_SYNC_TOKEN deve combaciare con QTS_SYNC_TOKEN configurato sul
# backend ENRI (endpoint dedicato /api/external/pratica-status, sola lettura).
...
async def list_concomitanza_enri(sess: dict = Depends(_require_staff_session)):
    ...
    ENRI (che gestisce la pratica per queste tratte, non Sertori/QTS) — una
...
async def get_scavi_timeseries(
    ...
    tempo' (scavi.html). Filtro opzionale per lotto e per intervallo date
    inclusivo (formato YYYY-MM-DD, confrontabile come stringa grazie al
    formato ISO); se omesse copre l'intero storico. Aggregazione fatta lato
    Mongo per evitare N+1 fetch (un cantiere per pratica, ciascuno col
    proprio storico log)."""
    pipeline: list = []
    if lotto:
