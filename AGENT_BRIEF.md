...
[x] Deploy Render confermato "Live" dopo fix `PYTHON_VERSION=3.11.10`
(rev.5/rev.6) — verificato dai deploy logs (`Deploy live for 6158694: Update AGENT_BRIEF.md`).
[x] Distribuire Code.gs come Web App sul Google Sheet QTS — fatto,
URL `/exec` ottenuto: `https://script.google.com/macros/s/ AKfycby_5hYBzumFCVCqIkQ3Qq27Q6_xcoHPBzaK0K59b7uH69cFg1EdG3tIjlSPQl3wCfk0Mg/exec`.
...
[~] Inserire su Render `APPS_SCRIPT_URL` (vedi sopra) e
...
[x] Testare login da `hub.html` con un nome/codice presente nel
foglio Utenti QTS — ok, login funzionante end-to-end (rev.9).
[x] Aggiornare `DEFAULT_API_BASE` in `js/api-config.js` e
...
[x] Aggiornare link GitHub Pages in `hub.html` — l'unico link
...
[ ] Caricare i file dati reali del progetto QTS (Master.csv,
...
[ ] Gantt: `.mpp` reale di QTS ancora da fornire — `GANTT_ROWS`/
`GANTT_DEPS` in `gantt.html` sono ancora quelli ereditati da ENRI,
vanno ricostruiti da zero una volta ricevuto il file.
[ ] Verificare se esiste un `brandkitretelit.md` dedicato o se va
riportato dal progetto ENRI.
[ ] Lotti/cluster: verificare se la struttura lotti (1A/1B/2A/2B) è la
...
qui — nei log d'avvio compare `[sync_cantieri] errore lettura master: 404: Master.csv not found`, coerente con §5.7 (dati reali ancora da
...
`py_compile` n/a (nessun file Python toccato), HTML parser + `node --check` OK su `scavi.html`.
